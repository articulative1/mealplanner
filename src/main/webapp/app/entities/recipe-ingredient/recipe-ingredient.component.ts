...
export class RecipeIngredientComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.recipeIngredientService
            ...
            .subscribe(
                (res: HttpResponse<IRecipeIngredient[]>) => this.paginateRecipeIngredients(res.body, res.headers),
    ...
    trackId(index: number, item: IRecipeIngredient) {
        return item.id;
    }

    registerChangeInRecipeIngredients() {
        this.eventSubscriber = this.eventManager.subscribe('recipeIngredientListModification', response => this.reset());
    }
    ...
    protected paginateRecipeIngredients(data: IRecipeIngredient[], headers: HttpHeaders) {
